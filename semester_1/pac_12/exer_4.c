...
int main(){
    ...
    int m; fscanf(in, "%d\n", &m);
    //hash_for_table 2
    int* hash_table_2 = (int*) malloc(sizeof(int)*m);
    //scan table 2
    ...
    for(int i=0; i<m; i++){
        ...
        hash_table_2[i] = string_hash(table_inMovie[i]->ActorName);
    }
    // JOIN
    Join_result** join_table_3 = (Join_result**) malloc((100001)*sizeof(Join_result*));
    int cur_index_in_3=0;
    int cur_hash;
    for(int i=0; i<n; i++){
        cur_hash = string_hash(table_acter_bio[i]->Name);
        for(int j=0; j<m; j++){
            if(cur_hash==hash_table_2[j]){
