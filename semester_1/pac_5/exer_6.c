...
LongNum int_to_long_num(int cur){
    LongNum num;
    num.len=0;
    int nDigits = floor(log10(abs(cur))) + 1;
...
{   
    ...
    scanf("%d", &n);
    LongNum fct, cur;
    fct.len=1, fct.arr[0]=1;
    for(int i=1; i<n+1; i++){
        //make from small number lon_num struct
        cur = int_to_long_num(i);
        //search next factorial
        fct = long_mult(fct, cur);
    }
    print_long(fct);
