...
int bitsetAny ( const bitword * arr , int left , int right ){
    ...
    if((right/64 - left/64) == 0 ){
        for(int i=left; i<right; i++){
            // cur_bit = (arr[i/64] >> (i%64)) & ((bitword) 1);
    ...
    //check end
    // temp = arr[right/64] << ( ( (bitword) 64) - (right%64) );
    // if(temp!=0) return 1;
...
int main(){
    ...
    int n; fscanf(in, "%d", &n);
    //varibles
    int t, size, index, new_val, left, right;
    bitword rez_bit;
    //arr for bitwords
    ...
    for(int oper = 0; oper<n; oper++){
        ...
        if(t==3){ // bitsetAny
            ...
            // printf("[%d,  %d) - %d", left, right, rez_bit);
            if(rez_bit==1){
                fprintf(out, "some\n");
            }else
            if(rez_bit==0){
